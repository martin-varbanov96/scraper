...
path_to_scraped_data = "/home/martin/Documents/project_baidu/scraper/logo_scrape/data_sets/201801210024_full_data_afterreimplementation.csv"
df_scraped_data = pd.read_csv(path_to_scraped_data)
df_scraped_data = df_scraped_data["site_id"].tolist()
...
def get_data():
    ...
    with open(path) as f:
        ...
        for row in data_read:
            ...
            output_list.append([el_https, el_http])
    return output_list
...
class LogoSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for el in get_data():
            tmp_flag = 0
            for protocol in el:
                if protocol not in df_scraped_data:
                    tmp_flag += 1
            if tmp_flag:
                yield scrapy.Request(el[0], self.parse)
