...
def get_data():
    path = 'logo_scrape/static/100_data.csv'
    output_list = list()
    with open(path) as f:
...
class LogoSpider(scrapy.Spider):
    ...
    def start_requests(self):
        self.site_id = 0
        # for el in ["https://beni4u.com"]:
        #     self.allowed_domains.append(el[8:]+"/")
        #     yield scrapy.Request(el, self.parse)
        for el in get_data():
            self.allowed_domains.append(el[7:])
        for el in get_data():
            yield scrapy.Request(el, self.parse)
    ...
    def parse_inner_page(self, response):
        ...
        xpath_img_patterns = [response.css("#Header").xpath("//a/img/@src"),
                              ...
                              response.xpath(
                                  "//a[@href='"+response.url+'/'+"']/img/@src"),
                              response.css("header").xpath(
                                  "//a[@href='/']/img/@src"),
        ...
        for xpath_pattern in xpath_img_patterns:
            ...
            if xpath_pattern:
                ...
                # self.link_id = "haaaable"
                print("*"*50, "Before yield", "*"*20)
                print(self.link_id)
                print(self.link_img)
                yield LogoScrapeItem(site_id=self.site_id,
                                     link_id=self.link_id,
